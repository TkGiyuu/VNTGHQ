  ...
  <div class="container-fluid px-4">
    ...
    <div class="collapse navbar-collapse" id="navMain">
      ...
      <ul class="navbar-nav align-items-center gap-1 ms-3">
        ...
        <li class="nav-item">
          <?php
          $isSeller = !empty($_SESSION['user_id']) && (function(){
            try { require_once __DIR__.'/db.php'; $r=db()->prepare('SELECT is_seller FROM users WHERE id=? LIMIT 1'); $r->execute([$_SESSION['user_id']]); $row=$r->fetch(); return !empty($row['is_seller']); } catch(Exception $e){ return false; }
          })();
          ?>
          <a class="nav-link nav-icon-btn <?= in_array($currentPage,['seller_dashboard','my_listings','create_listing','seller_settings'])?'active':'' ?>"
             href="<?= $isSeller ? 'seller_dashboard.php' : 'become_seller.php' ?>">
            <i class="bi bi-shop<?= $isSeller?'-window':'' ?>"></i>
            <span class="nav-label"><?= $isSeller?'My Shop':'Sell' ?></span>
          </a>
        </li>
        <li class="nav-item">
