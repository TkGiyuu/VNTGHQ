...
// Fetch real listings from sellers
try {
    $liveListings = db()->prepare(
        "SELECT l.id, l.title, l.price, l.img1, l.category, l.condition_val, l.views,
                u.seller_shop_name, u.username AS seller_username
         FROM listings l JOIN users u ON u.id=l.seller_id
         WHERE l.status='active'
         ORDER BY l.created_at DESC LIMIT 10"
    );
    $liveListings->execute();
    $liveListings = $liveListings->fetchAll();
} catch(Exception $e){ $liveListings = []; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="section-header d-flex justify-content-between align-items-center mb-3">
  <h2 class="section-title"><i class="bi bi-shop-window me-2 text-success"></i>New from Sellers</h2>
  <a href="homepage.php#top-picks" class="see-more">Browse all →</a>
</div>
<div class="row g-3 mb-5">
  <?php foreach($liveListings as $item): ?>
    ...
    <a href="listing.php?id=<?= $item['id'] ?>" class="text-decoration-none">
      <div class="product-card h-100 card-visible">
        <div class="product-img-wrap">
          ...
          <span style="position:absolute;bottom:8px;left:8px;background:var(--green);color:white;font-size:9px;font-weight:700;padding:2px 8px;border-radius:50px;letter-spacing:.5px">LIVE</span>
        </div>
        <div class="product-info">
          <p class="product-name"><?= htmlspecialchars($item['title']) ?></p>
          <div class="d-flex justify-content-between align-items-center">
            <span class="product-price">₱<?= number_format($item['price'],0) ?></span>
            <span class="text-muted" style="font-size:10px"><i class="bi bi-shop me-1"></i><?= htmlspecialchars($item['seller_shop_name'] ?: $item['seller_username']) ?></span>
          </div>
        </div>
      </div>
    </a>
