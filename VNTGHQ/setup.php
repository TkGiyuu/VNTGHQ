...
try {
    ...
    // ── Add seller columns to users (safe with IF NOT EXISTS logic via ALTER IGNORE) ──
    $pdo->exec("ALTER TABLE `users`
        ADD COLUMN IF NOT EXISTS `is_seller`        TINYINT(1)   NOT NULL DEFAULT 0     AFTER `is_admin`,
        ADD COLUMN IF NOT EXISTS `seller_bio`        TEXT                                AFTER `is_seller`,
        ADD COLUMN IF NOT EXISTS `seller_shop_name`  VARCHAR(120) DEFAULT NULL           AFTER `seller_bio`,
        ADD COLUMN IF NOT EXISTS `seller_rating`     DECIMAL(3,2) NOT NULL DEFAULT 0.00 AFTER `seller_shop_name`
    ");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS `listings` (
        ...
        `condition_val` ENUM('Brand New','Like New','Good','Fair','For Parts') NOT NULL DEFAULT 'Good',
        `img1`         TEXT,
        `img2`         TEXT,
        `img3`         TEXT,
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS `seller_reviews` (
        ...
        `comment`    TEXT,
        `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uq_review (`seller_id`,`buyer_id`,`listing_id`),
        FOREIGN KEY (`seller_id`) REFERENCES `users`(`id`) ON DELETE CASCADE,
...
/* NOTE: If you already ran setup.php before, run this SQL in phpMyAdmin to add seller tables:

ALTER TABLE users ADD COLUMN is_seller TINYINT(1) NOT NULL DEFAULT 0 AFTER is_admin;
ALTER TABLE users ADD COLUMN seller_bio TEXT AFTER is_seller;
ALTER TABLE users ADD COLUMN seller_shop_name VARCHAR(120) DEFAULT NULL AFTER seller_bio;
ALTER TABLE users ADD COLUMN seller_rating DECIMAL(3,2) DEFAULT 0.00 AFTER seller_shop_name;

CREATE TABLE IF NOT EXISTS listings ( ... ) -- see setup.php for full SQL
